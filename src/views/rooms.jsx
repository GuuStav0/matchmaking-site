  ...
  return (
    <div className="slot-bar-wrap">
      <div className="slot-bar-track">
        <div className="slot-bar-fill" style={{ width: `${pct}%`, background: color }} />
      </div>
      <span className="slot-text" style={{ color }}>
        {members}<span className="slot-sep">/</span>{slots}
      </span>
    </div>
  );
}
    ...
    >
      <div className="room-card__top">
        <div className="room-card__left">
          <div className="room-card__owner-row">
            <div className="room-card__avatar">
              {room.owner && typeof room.owner === "string" && room.owner.length > 0
          ...
          <div className="room-card__tags">
            <span className={`room-card__style-badge room-card__style-badge--${room.style === "Competitivo" ? "comp" : "casual"}`}>
              {room.style === "Competitivo" ? "⚔️" : "🎮"} {room.style}
            </span>
            {room.mic_required && <span className="room-card__mic-badge">🎙️ Mic</span>}
        ...
        <div className="room-card__right">
          ...
          {!full && (
            <button className="room-card__btn" onClick={(e) => { e.stopPropagation(); onClick(room); }}>
              Ver detalhes
            </button>
          )}
        </div>
...
function SlidePanel({ room, onClose }) {
  ...
  const handleJoinRoom = async () => {
    ...
    try {
      const data = await request("/group-members", {
        method: "POST",
        body: JSON.stringify({ group_id: parseInt(room.id), profile_id: parseInt(profileId) }),
      });

      if (data.ok || data.status === "sucesso") {
        setPopupType("success");
        setPopupMessage("Acesso autorizado! Entrando no lobby tático...");
        setPopupOpen(true);
        setTimeout(() => {
          setPopupOpen(false);
          navigate(`/rooms/${gameId}/${room.id}`);
        }, 1500);
      } else {
        setPopupType("error");
  ...
  return (
    <>
      <div className={`panel-overlay ${mounted ? "panel-overlay--visible" : ""}`} onClick={onClose} />
      <aside ref={panelRef} className={`slide-panel ${mounted ? "slide-panel--open" : ""}`} role="dialog" aria-label={`Detalhes: ${room.name}`}>
        <div className="sp-header">
          <div className="sp-header__top">
            <span className={`sp-style-badge sp-style-badge--${isComp ? "comp" : "casual"}`}>
        ...
        <div className="sp-section">
          ...
          <div className="sp-slots">
            {slots.map((filled, i) => (
              <div key={i} className={`sp-slot ${filled ? "sp-slot--filled" : "sp-slot--empty"}`}>
                {filled ? (
          ...
          <div className="sp-slots-label" style={{ color: barColor }}>
            ...
            {room.slots - room.members} vaga{room.slots - room.members !== 1 ? "s" : ""} restante{room.slots - room.members !== 1 ? "s" : ""}
          </div>
        </div>
        ...
        <div className="sp-section">
          ...
          <div className="sp-info-grid">
            {[
              ...
              { icon: "🎙️", label: "Microfone", value: room.mic_required ? "Obrigatório" : "Opcional" },
            ].map((item) => (
        ...
        <div className="sp-footer">
          {full ? (
            <div className="sp-full-notice">Esta sala está cheia no momento.</div>
          ) : (
      ...
      <Popup isOpen={popupOpen} message={popupMessage} type={popupType} onClose={() => setPopupOpen(false)} />
    </>
  );
}
...
export default function RoomsPage() {
    ...
    async function loadData() {
      try {
        const [gameData, roomsData] = await Promise.all([
          ...
          request(`/games/${gameId}/rooms`),
        ]);
        setGame(gameData);
        setRooms(Array.isArray(roomsData) ? roomsData : (roomsData.dados ?? []));
      } catch {
        // erro de rede — mantém estado vazio
      } finally {
        setLoading(false);
      }
    }
  ...
  const filtered = rooms
    .filter((r) => {
      ...
      const matchSlot   = slotFilter === "Todos"
        || (slotFilter === "Com vaga"    && r.members < r.slots)
        || (slotFilter === "Quase cheio" && r.slots - r.members <= 1 && r.members < r.slots);
      return matchSearch && matchStyle && matchSlot;
    })
    .sort((a, b) =>
      sortBy === "slots" ? a.slots - a.members - (b.slots - b.members) : a.id - b.id
    );
    ...
    <div className="rooms-page">
      ...
      <main className="rooms-main">
        <div className="rooms-breadcrumb">
          <button className="rooms-breadcrumb__back" onClick={() => navigate("/games")}>
        ...
        <section className="rooms-hero">
          <div className="rooms-hero__cover" style={{ background: `linear-gradient(135deg, ${game?.cover_color}, #0f0f1a)` }}>
            <span className="rooms-hero__cover-initial" style={{ color: game?.cover_accent }}>
          ...
          <div className="rooms-hero__info">
            ...
            <div className="rooms-hero__stats">
              ...
              <div className="rooms-hero__stat">
                <span className="rooms-hero__stat-num" style={{ color: "#22c55e" }}>{openCount}</span>
                <span className="rooms-hero__stat-lbl">com vaga</span>
              </div>
              <div className="rooms-hero__stat-div" />
              <div className="rooms-hero__stat">
                <span className="rooms-hero__stat-num">{rooms.reduce((a, r) => a + r.members, 0)}</span>
                <span className="rooms-hero__stat-lbl">jogadores ativos</span>
              </div>
            </div>
          </div>
          <button className="rooms-hero__create-btn" onClick={() => navigate(`/rooms/${gameId}/create`)}>
            + Criar sala
          </button>
        </section>
        ...
        <section className="rooms-controls">
          <div className="rooms-controls__row">
            <div className="rooms-search">
              <svg className="rooms-search__icon" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              />
              {search && <button className="rooms-search__clear" onClick={() => setSearch("")}>✕</button>}
            </div>

            <div className="rooms-sort">
              <span className="rooms-sort__lbl">Ordenar:</span>
              {[["recent", "Recentes"], ["slots", "Mais vagas"]].map(([v, l]) => (
                <button key={v} className={`rooms-sort__btn ${sortBy === v ? "rooms-sort__btn--on" : ""}`} onClick={() => setSort(v)}>
                  {l}
                </button>
              ))}
            </div>
          ...
          <div className="rooms-filters">
            <div className="rooms-filter-group">
              ...
              <div className="rooms-pills">
                {STYLE_FILTERS.map((f) => (
                  <button key={f} className={`rooms-pill ${styleFilter === f ? "rooms-pill--on" : ""}`} onClick={() => setStyle(f)}>{f}</button>
                ))}
              </div>
            </div>
            <div className="rooms-filter-group">
              ...
              <div className="rooms-pills">
                {SLOT_FILTERS.map((f) => (
                  <button key={f} className={`rooms-pill ${slotFilter === f ? "rooms-pill--on" : ""}`} onClick={() => setSlot(f)}>{f}</button>
                ))}
              </div>
            </div>
          </div>
        ...
        <div className="rooms-meta">
          <span>{filtered.length} sala{filtered.length !== 1 ? "s" : ""} encontrada{filtered.length !== 1 ? "s" : ""}</span>
          <span>{filtered.filter((r) => r.members < r.slots).length} com vagas disponíveis</span>
        </div>

        {filtered.length > 0 ? (
          <div className="rooms-list">
            {filtered.map((room, i) => (
              <div key={room.id} className="rooms-list__item" style={{ animationDelay: `${i * 55}ms` }}>
                <RoomCard room={room} onClick={setSelected} active={selectedRoom?.id === room.id} />
              </div>
            ))}
          </div>
        ) : (
          <div className="rooms-empty">
            ...
            <p className="rooms-empty__sub">Tente limpar os filtros ou seja o primeiro a criar uma sala!</p>
            <button className="rooms-empty__btn" onClick={() => { setSearch(""); setStyle("Todos"); setSlot("Todos"); }}>
              Limpar filtros
            </button>
          </div>
        )}
      ...
      {selectedRoom && <SlidePanel room={selectedRoom} onClose={() => setSelected(null)} />}
