...
export default function RoomDetail() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setPageState("ready");
        triggerPopup(`Bem-vindo à sala "${roomData.name}"! 🎮`, "success");
      } catch (err) {
  ...
  useEffect(() => {
    if (room) {
      document.title = `${room.name} · ${members.length}/${room.max_slots ?? room.slots} jogadores`;
    }
    return () => { document.title = "Matchmaking"; };
  }, [room, members]);
