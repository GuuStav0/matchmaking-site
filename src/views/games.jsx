...
function GameCard({ game, index }) {
  ...
  // Fallback seguro caso a API traga cover_url ou cover
  const imagemCapa = game.cover_url || game.cover;
  // Fallback seguro para a contagem de salas abertas vindas da API
  const totalSalas = game.rooms_count !== undefined ? game.rooms_count : (game.rooms || 0);
    ...
    >
      ...
      <div className="game-card__info">
        ...
        <div className="game-card__rooms">
          ...
          <span className="game-card__rooms-count">{totalSalas.toLocaleString("pt-BR")}</span>
...
export default function GamesDirectory() {
  ...
  const filtered = games
    ...
    .sort((a, b) => {
      ...
      return sortBy === "rooms" ? salasB - salasA : a.name.localeCompare(b.name);
    });

  const totalRooms = filtered.reduce((acc, g) => acc + (g.rooms_count !== undefined ? g.rooms_count : (g.rooms || 0)), 0);
  const totalSalasGerais = games.reduce((acc, g) => acc + (g.rooms_count !== undefined ? g.rooms_count : (g.rooms || 0)), 0);
      ...
      <main className="dir-main">
          ...
          <div className="dir-hero__inner">
            <div className="dir-hero__text">
              ...
              <p className="dir-hero__sub">
                ...
                <strong>{totalSalasGerais.toLocaleString("pt-BR")}</strong> salas abertas agora
              </p>
            </div>
          </div>
        ...
        {loadingReal ? (
          <div className="dir-empty">
            ...
            <p className="dir-empty__sub">Buscando a lista de jogos atualizada no servidor.</p>
          </div>
        ) : erroApi ? (
          <div className="dir-empty">
